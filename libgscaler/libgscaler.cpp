...
{
    int __unused i     = 0;
    int __unused op_id = 0;
    unsigned int __unused total_sleep_time  = 0;
    int ret = 0;
...
int exynos_gsc_subdev_s_crop(void *handle,
        exynos_mpp_img * __unused src_img, exynos_mpp_img *dst_img)
{
...
int exynos_gsc_free_and_close(void *handle)
{
    ...
    struct v4l2_buffer __unused buf;
    struct v4l2_plane __unused  planes[NUM_OF_GSC_PLANES];
    int __unused ret = 0;
    CGscaler* gsc = GetGscaler(handle);
    if (gsc == NULL) {
        ALOGE("%s::handle == NULL() fail", __func__);
